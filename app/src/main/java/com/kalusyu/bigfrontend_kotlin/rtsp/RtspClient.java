package com.kalusyu.bigfrontend_kotlin.rtsp;
...
import com.kalusyu.bigfrontend_kotlin.rtsp.rtspclinet.socket.RtpSocket;
import com.kalusyu.bigfrontend_kotlin.rtsp.rtspclinet.video.H264Stream;
...
public class RtspClient {
    ...
    private void sendRequestSetup() throws IOException {
        ...
        String transport = mResponse.headers.get("transport");
        String tr1 = transport.replace("source=3.84.6.190;", "");

        if(isTCPtranslate) matcher = Response.regexTCPTransport.matcher(mResponse.headers.get("transport"));
        else matcher = Response.regexUDPTransport.matcher(tr1);
        if(matcher.find()) {
